...
export class ApiService {
  ...
  async getPromocionByUserId(userId: number): Promise<{
    id: number;
    nombre: string;
    ...
    const promocion = (
      await axiosAPIPromociones.get(config.APIPromocionesUrls.getPromocionByUserId(userId))
    ).data;
    return promocion;
  }

  async validarPromocionDeUsuario(): Promise<any> {
    const response = await axiosAPIPromociones.get(config.validarPromocionDeUsuario);
    return response.data;
  }
}
